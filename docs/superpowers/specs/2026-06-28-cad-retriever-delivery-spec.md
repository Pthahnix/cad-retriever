...
> 本节固化评估对象、划分协议、目标指标（数据驱动留白 + 相对止损线）、统计方法与坍缩检测规范。Part 9 train / build-artifact 单元的验收标准以本节为准；与本节有冲突的其他描述均以本节覆盖。
...
本节是 §5.5 划分策略的**评估侧固化版本**，明确执行规范；与 §5.5 有冲突时以本节为准（本节为评估权威来源）。
...
- 判据为**充要条件**之一：差异需 ΔTop-1 ≥ 2 pp，**且**阶段 D 换种子后差异仍成立，方判定为稳健差异。
