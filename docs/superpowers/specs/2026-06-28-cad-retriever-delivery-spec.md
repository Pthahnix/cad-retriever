...
单分支方案（仅草图 ↔ 渲染图）在区分几何相近的零件时容易在视角遮挡处丢失判别信息；双分支 + 可选 B-rep 可以用精确拓扑信息弥补投影损失。这是**任务需求与架构设计之间的结构性匹配**，而非凭偏好选择。
...
使用 PhotoSketch 从 CAD 多视图渲染图自动生成训练草图，直接解决了合成草图（如 Canny 边缘图、几何线框图）与真实手绘草图之间的**域偏差（Domain Gap）**问题。PhotoSketch 是数据管线中的核心组件，不是可选优化。
...
- [ ] Astro 前端可通过浏览器访问，草图上传后在 3 秒内渲染出 Top-10 结果卡片（以 p50 为目标，见 Part 7）。
...
| `deduplicate_ratio` | 去重比例参数，`deduplicate_ratio = 2`，即原始 ABC chunk1 数据经去重后保留约 1/2（来源：ABC_V2 数据集处理记录） |
