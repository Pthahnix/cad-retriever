...
```
                 ┌──────────────────────────────────────────────┐
                 │ M3 阶段 A 坍缩探针：只需 views+sketches，      │
                 │ 可早于 M2 完成即启动（不依赖 graph*.pt）       │
                 └───────────────────────────┬──────────────────┘
                                             │（提前并行）
 M0 ───► M1 ──────────────► M2 ──────────► M3 ───► M4 ───► M5 ───► M6
 环境     数据落地           B-rep 图       消融     建库     服务     前端
 │        │                  ▲              决策     产物     上线     联调
 │        │  probe 完成      │                                ▲        ▲
 │        └──────────────────┘                                │        │
 │           （M2 提取器在 M1 probe 后即可并行开发）           │        │
 │                                                            │        │
 │   M5 Rust 脚手架可对 mock artifact 先写 ───────────────────┘        │
 │                                                                     │
 └── M6 可对 mock /search 先接（serve 真接前先用假数据联调前端）───────┘

阶段 B 内部依赖：
   R1（SE-ResNet50 / CNN-only）── 不需 graph*.pt，可在 M2 完成前先跑
   R3（SE-ResNet50 / BOTH）   ── 需 graph*.pt，必须在 M2 完成后跑
```
...
**graph_status 取值**（manifest）：

| 值 | 含义 | 下游处理 |
|----|------|----------|
| `ok` | 图成功提取并写入 `graph*.pt` | BOTH 分支正常用其图特征 |
| `cnn_only_fallback` | 提取失败，不写入 `graph*.pt` | data loader 据此零填充图特征分支 |
