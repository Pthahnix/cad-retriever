...
仓库目录**不存储**任何超过 10 MB 的文件。大数据路径在各 Part 中以 `~/data/<子目录>` 形式给出，工程师在实际执行时映射到 GPU 机对应挂载点。
...
| **(b) 数据驱动留白 + 相对止损线** | 值依赖消融结果，不预设，但给出相对于基线的可接受下界 | Top-1 目标：优于 CNN 双分支基线；止损线：不低于基线 −3 pp |
...
在 M3 完成之前，"检索性能目标"一栏保持形式 **(b)**：留白 + 止损线（不低于文献报告的可比方法基线）。
