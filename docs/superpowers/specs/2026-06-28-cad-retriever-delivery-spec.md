...
└──────────────────────────────────┘
        ...
        ▼                                         ▼
┌──────────────────────┐            ┌─────────────────────────────┐
│  训练（消融选主干）  │            │  STEP 拓扑 → B-rep UV-grid  │
│  · CNN 双分支 + 域   │            │  → graph*.pt                │
│    对抗（默认基线）  │            │  · 失败模型写回退标记        │
│  · [可选] B-rep GNN  │◄───────────┤  · 永不进入 ONNX 导出路径   │
│  · [可选] CLIP-ViT   │            └─────────────────────────────┘
└──────────────────────┘
        │ 训练完成，锁定 CAD 编码器权重
        ▼
...
`serve` 启动时校验上述三者，若不匹配则拒绝启动并输出明确错误信息。防止因建库与导出步骤不同步导致的静默错误。
...
| **B-rep GNN 离线-only**：GNN 只参与训练和全库提特征，不导出 ONNX | PyG `scatter_reduce` 等操作是 ONNX 导出雷区，强行导出需大量算子适配；GNN 输出已融合进 512-d 向量写盘，serve 无需感知 GNN 存在 | build（GNN 隔离）、serve（不依赖 PyG） |
